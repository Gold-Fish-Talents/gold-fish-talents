...
async function onSignUp() {
  ...
  console.log('[Register] Executing authentication...')
  await execute()
  ...
  if (showOTP.value && data.value?.isSuccess && !data.value?.navigateTo) {
    console.log('[Register] OTP Verified but no navigateTo, forcing fallback')
  ...
  if (data.value?.navigateTo) {
    console.log('[Register] Navigating to:', data.value.navigateTo)
    ...
    console.log('[Register] Success but no navigateTo, using fallback: /jobs/post-job')
    await navigateTo('/jobs/post-job')
  } else if (error.value) {
    console.error('[Register] Authentication error:', error.value)
  }
}

function onOauthSignUp() {
  console.log('OAuth sign up requested')
}
  ...
  <main class="relative flex min-h-screen w-full overflow-hidden bg-[#030719]">
    ...
    <div class="container mx-auto flex h-screen items-center justify-center px-4 pt-20">
      ...
      <section class="relative z-10 w-full max-w-[420px] rounded-3xl border border-white/10 bg-white/5 p-8 shadow-2xl backdrop-blur-xl">
        ...
        <form class="flex flex-col gap-5" novalidate @submit.prevent="onSignUp">
          <!-- Email -->
        ...
        <!-- Footer -->
        <div class="text-gray-400 mt-8 text-center text-sm">Already have an account? <NuxtLink to="/auth/client-login" class="font-semibold text-[#FFD400] hover:underline">Sign In</NuxtLink></div>
      </section>
